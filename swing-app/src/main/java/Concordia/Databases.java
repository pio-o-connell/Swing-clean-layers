...
import Concordia.annotations.Singleton;
...
public final class Databases {
    ...
    public void deleteItemTransintoDatabase(Connection con, ArrayList<Company> companies, int companyIndex, int itemIndex, int historyIndex) throws SQLException {
        try {
            ArrayList<Item> itemPointer = companies.get(companyIndex).getItems();
            itemPointer.remove(itemIndex);
            // remove from databases
            // item database
            String query = "delete from item where Item_id = ?";
            PreparedStatement preparedStmt1 = (PreparedStatement) con.prepareStatement(query);
            preparedStmt1.setInt(1, itemPointer.get(historyIndex).getItemId());
            ...
            preparedStmt11.setInt(1, itemPointer.get(itemIndex).getItemId());
    ...
    public void deleteHistoryTransintoDatabase(Connection con, ArrayList<Company> companies, int companyIndex, int itemIndex, int historyIndex) throws SQLException {
        try {
            ArrayList<history> historyPointer = companies.get(companyIndex).getItems().get(itemIndex).getHistoryItem();
            System.out.println("history  pointer..>" + historyPointer.get(historyIndex).getHistoryId());
            historyPointer.remove(historyIndex);
            // remove from history database
            String query = "delete from history where history_id = ?";
            PreparedStatement preparedStmt = (PreparedStatement) con.prepareStatement(query);
            preparedStmt.setInt(1, historyPointer.get(historyIndex).getHistoryId());
    ...
    public void insertTransactionintoDatabase(Connection con, int itemId, int amount, String location, String supplier, String delivery, String notes) throws SQLException {
        try {
            ...
            // Any updates to in-memory objects should be handled by the service/controller layer
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public void insertNewItemTransintoDatabase(Connection con, int companyId, int amount, String itemName, String notes, String location, String supplier, String delivery) throws SQLException {
        try {
        ...
        try {
            ...
            statement.setString(6, notes);
            statement.executeUpdate();
            // Now to retrieve the auto generated history_id if needed
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
